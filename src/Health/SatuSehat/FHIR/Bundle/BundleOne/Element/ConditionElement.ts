...
import {
  ...
  FHIRBundleOneModelConfigClinicalStatus,
  FHIRBundleOneModelConfigEntryCondition,
...
interface SatuSehatFunctionClassParsingBundleCondition
  extends SatuSehatFunctionClassParsing {
...
export class BundleOneConditionElementClasses {
  ...
  constructor(options: SatuSehatFunctionClassParsingBundleCondition) {
    ...
    const namaPasien =
      BundleOneConditionElementClasses.encounter.resource.subject !== undefined
        ? BundleOneConditionElementClasses.encounter.resource.subject.display
        : 'Pasien';
    const tanggalKunjunganTerakhir =
      BundleOneConditionElementClasses.encounter.resource.period?.end !==
      undefined
        ? moment(
            BundleOneConditionElementClasses.encounter.resource.period?.end,
  ...
  addSubject(subject?: FHIRBundleOneModelConfigSubject) {
    if (subject === undefined) {
      BundleOneConditionElementClasses.model.resource.subject =
        BundleOneConditionElementClasses.encounter.resource.subject;
    } else {
      subject.reference =
        typeof subject.reference === 'number'
  ...
  addClinicalStatus(clinicalStatus: FHIRBundleOneModelConfigClinicalStatus) {
    BundleOneConditionElementClasses.model.resource.clinicalStatus =
      clinicalStatus;
    return this;
  }
  ...
  submit() {
    ...
    const DiagnosaNum = BundleOneClasses.EntryData.filter(
      (data) => data.resource.resourceType === 'Condition',
    ).length;
    if (
      BundleOneConditionElementClasses.encounter.resource.diagnosis?.[
        DiagnosaNum - 1
      ...
      BundleOneConditionElementClasses.encounter.resource.diagnosis[
        ...
        `${BundleOneClasses.EntryData[DiagnosaNum].fullUrl}`;
    }
