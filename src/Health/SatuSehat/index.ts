...
import {
  SatuSehatCallbackProduction,
} from './Interfaces/SatuSehatCallback.type';
    ...
    return new Promise((resolve, rejected) => {
      if (this.finalConfig.credentials?.auth?.clientId === undefined)
        return rejected({
          status: false,
          code: 400,
          msg: `clientId is require not found`,
        });
      if (this.finalConfig.credentials?.auth?.clientSecret === undefined)
        return rejected({
          status: false,
          code: 400,
          msg: `clientSecret is require not found`,
        });
      if (this.HostConfig === undefined)
        return rejected({
          status: false,
          code: 400,
          msg: `host config undefined`,
        });
      ...
      axios<SatuSehatCallbackProduction>({
        ...
        .catch((error) => {
          rejected({
            ...
            error: error.response.data.issue,
          });
        });
    });
  }
